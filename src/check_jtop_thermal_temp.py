...
"""Check thermal temperature with jtop information on nvidia jetson devices."""
...
def get_thermal_temp(stats):
    """
    Get the thermal temperature from the statistics.

    Parameters:
    stats (dict): The statistics.

    Returns:
    int: The thermal temperature.
    """
    return stats["temperature"]["thermal"]


def check_condition(warni=40, critical=50):
    """
    Check the thermal temperature and return the status code, message, and performance data.
    ...
    """
    stats = get_stats()
    temp = get_thermal_temp(stats=stats)
    print(temp)
    if temp < warni:
        return {"code": "OK", "message": "thermal temperature is OK", "perfdata": str(temp)}
    elif temp >= warni and temp < critical:
        return {"code": "WARNING", "message": "thermal temperature is WARNING", "perfdata": str(temp)}
    elif temp >= critical:
        return {"code": "CRITICAL", "message": "thermal temperature is CRITICAL", "perfdata": str(temp)}
...
def main():
    ...
    output = f"{result['message']} | 'thermal Temperature'={result['perfdata']}°C;{args.warning};{args.critical};0;100"
