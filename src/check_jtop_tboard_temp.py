...
"""Check tboard temperature with jtop information on nvidia jetson devices."""
...
def get_tboard_temp(stats):
    """
    Get the Tboard temperature from the statistics.

    Parameters:
    stats (dict): The statistics.

    Returns:
    int: The Tboard temperature.
    """
    return stats["temperature"]["Tboard"]


def check_condition(warni=40, critical=50):
    """
    Check the Tboard temperature and return the status code, message, and performance data.
    ...
    """
    stats = get_stats()
    temp = get_tboard_temp(stats=stats)
    print(temp)
    if temp < warni:
        return {"code": "OK", "message": "Tboard temperature is OK", "perfdata": str(temp)}
    elif temp >= warni and temp < critical:
        return {"code": "WARNING", "message": "Tboard temperature is WARNING", "perfdata": str(temp)}
    elif temp >= critical:
        return {"code": "CRITICAL", "message": "Tboard temperature is CRITICAL", "perfdata": str(temp)}
...
def main():
    ...
    output = f"{result['message']} | 'Tboard Temperature'={result['perfdata']}°C;{args.warning};{args.critical};0;100"
